...
This corresponds to labeling the vertices of the anti-graph injectively in such a way that the labeling $f(uv) = \left\lfloor (u + v) / 2\right\rfloor\ (\text{mod}\ k)$ distinguishes all the anti-edges. I call graphs having this property *soft-mean*, by analogy to *mean graphs* which have a similar definition.
